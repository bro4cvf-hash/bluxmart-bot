...
function overwritesMatch(ch, desired) {
    try {
...
async function main() {
    ...
    client.once('clientReady', async () => {
        try {
            const guild = await client.guilds.fetch(config_1.config.guildId);
            await guild.roles.fetch();
            await guild.channels.fetch();
            const everyone = guild.roles.everyone;
            const R = (n) => guild.roles.cache.find((r) => r.name === n);
                ...
                try {
                    if (c.name.includes('STAFF')) {
                        const want = [
                            ...
                            ...staff.map((r) => ({ id: r.id, allow: [discord_js_1.PermissionFlagsBits.ViewChannel, discord_js_1.PermissionFlagsBits.SendMessages, discord_js_1.PermissionFlagsBits.ReadMessageHistory] })),
                        ];
            ...
            for (const [, ch] of guild.channels.cache) {
                ...
                try {
                    if (SENIOR_ONLY_NAMES.has(name)) {
                        const ow = [
                            { id: everyone.id, deny: [discord_js_1.PermissionFlagsBits.ViewChannel] },
                            ...senior.map((r) => ({ id: r.id, allow: [discord_js_1.PermissionFlagsBits.ViewChannel, discord_js_1.PermissionFlagsBits.SendMessages, discord_js_1.PermissionFlagsBits.ReadMessageHistory] })),
                        ];
                        if (muted)
                            ow.push({ id: muted.id, deny: [discord_js_1.PermissionFlagsBits.ViewChannel] });
                        ...
                        const ow = [
                            { id: everyone.id, allow: [discord_js_1.PermissionFlagsBits.ViewChannel, discord_js_1.PermissionFlagsBits.ReadMessageHistory], deny: [discord_js_1.PermissionFlagsBits.SendMessages, discord_js_1.PermissionFlagsBits.AddReactions] },
                            ...staff.map((r) => ({ id: r.id, allow: [discord_js_1.PermissionFlagsBits.ViewChannel, discord_js_1.PermissionFlagsBits.SendMessages, discord_js_1.PermissionFlagsBits.ReadMessageHistory] })),
                        ];
                        if (muted)
                            ow.push({ id: muted.id, allow: [discord_js_1.PermissionFlagsBits.ViewChannel], deny: [discord_js_1.PermissionFlagsBits.SendMessages, discord_js_1.PermissionFlagsBits.AddReactions] });
                        if (overwritesMatch(ch, ow)) {
                            console.log(`[fix] SKIP: ${name} (in sync)`);
                            continue;
                        }
                    ...
                    else if (VOICE_NAMES.has(name) || ch.type === discord_js_1.ChannelType.GuildVoice) {
                        const ow = [
                            { id: everyone.id, allow: [discord_js_1.PermissionFlagsBits.ViewChannel, discord_js_1.PermissionFlagsBits.Connect, discord_js_1.PermissionFlagsBits.Speak] },
                        ];
                        if (muted)
                            ow.push({ id: muted.id, deny: [discord_js_1.PermissionFlagsBits.Connect, discord_js_1.PermissionFlagsBits.Speak] });
                        if (overwritesMatch(ch, ow)) {
                            console.log(`[fix] SKIP: ${name} (in sync)`);
                            continue;
                        }
                ...
                for (const [, ch] of guild.channels.cache) {
                    try {
                        ...
                        if (ch.type === discord_js_1.ChannelType.GuildVoice) {
                            ...
                            if (ow && (deny & discord_js_1.PermissionFlagsBits.Connect) !== 0n && (deny & discord_js_1.PermissionFlagsBits.Speak) !== 0n)
                                continue;
                            await ch.permissionOverwrites.edit(muted, { Connect: false, Speak: false }).catch(() => { });
                            mutedFixed++;
                        }
                        else if (ch.isTextBased?.() && !ch.isDMBased?.()) {
                            const deny = (() => {
                                try {
                                    return ow?.deny.bitfield ?? 0n;
                                }
                            ...
                            if (ow && (deny & discord_js_1.PermissionFlagsBits.SendMessages) !== 0n)
                                continue;
