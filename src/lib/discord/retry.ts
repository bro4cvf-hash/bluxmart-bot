import {
  RetryConfigurationError,
  errorCode,
  errorMessage,
  errorName,
  errorRetryAfterMs,
  errorStatus,
  isAbortError,
  isOperationTimeoutError,
  toAbortError,
} from './errors';
import { waitForDelay, withAbort, type TimerScheduler } from './timeout';
import type {
  OperationError,
  RetryClassification,
  RetryClass,
} from './types';

export { RetryConfigurationError } from './errors';

const DEFAULT_MAX_ATTEMPTS = 3;
const MAX_ALLOWED_ATTEMPTS = 20;
const DEFAULT_BASE_DELAY_MS = 100;
const DEFAULT_MAX_DELAY_MS = 30_000;
const DEFAULT_BACKOFF_FACTOR = 2;

/** Injectable delay between attempts; tests can resolve it synchronously. */
export type RetryWaiter = (
  delayMs: number,
  signal: AbortSignal,
  scheduler?: TimerScheduler,
) => Promise<void>;

export const systemRetryWaiter: RetryWaiter = (delayMs, signal, scheduler) =>
  waitForDelay(delayMs, { signal, scheduler });

export interface RetryAttemptContext {
  /** One-based attempt number. */
  readonly attempt: number;
  readonly maxAttempts: number;
  readonly signal: AbortSignal;
}

export interface RetryContext {
  readonly error: unknown;
  readonly classification: RetryClassification;
  readonly attempt: number;
  readonly nextAttempt: number;
  readonly maxAttempts: number;
  readonly signal: AbortSignal;
  readonly delayMs: number;
}

export interface RetryOptions {
  /** Total attempts, including the first call. Defaults to 3. */
  readonly maxAttempts?: number;
  /** Alias for maxAttempts - 1. Do not provide both unless they agree. */
  readonly maxRetries?: number;
  readonly baseDelayMs?: number;
  readonly maxDelayMs?: number;
  readonly backoffFactor?: number;
  /** Honor a provider retry-after hint when present. Defaults to true. */
  readonly respectRetryAfter?: boolean;
  /** Fractional deterministic jitter in the range [0, 1]. Defaults to 0. */
  readonly jitter?: number;
  readonly random?: () => number;
  readonly wait?: RetryWaiter;
  readonly signal?: AbortSignal;
  readonly scheduler?: TimerScheduler;
  readonly shouldRetry?: (
    error: unknown,
    context: RetryContext,
  ) => boolean | Promise<boolean>;
  readonly onRetry?: (context: RetryContext) => void | Promise<void>;
}

function numericStatus(error: unknown): number | undefined {
  const status = errorStatus(error);
  return status !== undefined && Number.isFinite(status) && status >= 100
    ? status
    : undefined;
}

function normalizedCode(error: unknown): string {
  return (errorCode(error) ?? '').toUpperCase();
}

function normalizedName(error: unknown): string {
  return errorName(error).toUpperCase();
}

const NETWORK_CODES = new Set([
  'ECONNRESET',
  'ECONNREFUSED',
  'ECONNABORTED',
  'ETIMEDOUT',
  'ENOTFOUND',
  'EAI_AGAIN',
  'EPIPE',
  'EHOSTUNREACH',
  'ENETUNREACH',
  'ENETDOWN',
  'UND_ERR_SOCKET',
  'UND_ERR_CONNECT_TIMEOUT',
  'UND_ERR_HEADERS_TIMEOUT',
  'UND_ERR_BODY_TIMEOUT',
]);

function looksLikeNetworkError(error: unknown, status?: number): boolean {
  if (status !== undefined) return false;
  const code = normalizedCode(error);
  const name = normalizedName(error);
  if (NETWORK_CODES.has(code)) return true;
  if (
    name === 'FETCHERROR' ||
    name === 'NETWORKERROR' ||
    name === 'REQUESTERROR' ||
    name === 'TIMEOUTERROR' ||
    name === 'SOCKETERROR'
  ) {
    return true;
  }
  const message = errorMessage(error).toLowerCase();
  return (
    message.includes('fetch failed') ||
    message.includes('network error') ||
    message.includes('socket hang up') ||
    message.includes('connection reset') ||
    message.includes('connection refused') ||
    message.includes('timed out') ||
    message.includes('timeout') ||
    message.includes('temporarily unavailable')
  );
}

function makeClassification(
  retryable: boolean,
  kind: RetryClass,
  reason: string,
  error: unknown,
  status?: number,
): RetryClassification {
  const code = errorCode(error);
  const retryAfterMs = errorRetryAfterMs(error);
  return {
    retryable,
    kind,
    status,
    code,
    ...(retryAfterMs === undefined ? {} : { retryAfterMs }),
    reason,
  };
}

/**
 * Classify thrown values without importing a Discord client. HTTP 429 and 5xx,
 * network/timeout failures, and transient request timeouts are retryable.
 * Ordinary 4xx responses are permanent; caller cancellation is never retried.
 */
export function classifyRetryableError(error: unknown): RetryClassification {
  const status = numericStatus(error);
  const code = normalizedCode(error);
  const name = normalizedName(error);

  // Prefer an explicit HTTP response over an incidental error code. A 400/403
  // response is permanent even if a client happened to attach ETIMEDOUT.
  if (status === 429 || (status === undefined && (code === 'TOO_MANY_REQUESTS' || code === 'RATE_LIMITED'))) {
    return makeClassification(
      true,
      'rate-limit',
      'The remote service reported a rate limit.',
      error,
      status,
    );
  }

  if (status !== undefined && status >= 500 && status <= 599) {
    return makeClassification(
      true,
      'server',
      'The remote service reported a server error.',
      error,
      status,
    );
  }

  // 408 and 425 are request-level transient failures even though their HTTP
  // status is in the 4xx range. All other 4xx responses are permanent.
  if (status === 408 || status === 425) {
    return makeClassification(
      true,
      'network',
      'The request was transiently unavailable.',
      error,
      status,
    );
  }

  if (status !== undefined && status >= 400 && status <= 499) {
    return makeClassification(
      false,
      'client',
      'The remote service rejected the request permanently.',
      error,
      status,
    );
  }

  if (isOperationTimeoutError(error) || name === 'TIMEOUTERROR' || code === 'ETIMEDOUT') {
    return makeClassification(
      true,
      'timeout',
      'The operation reached its local or request timeout.',
      error,
      status,
    );
  }

  if (isAbortError(error)) {
    return makeClassification(
      false,
      'aborted',
      'The caller cancelled the operation.',
      error,
      status,
    );
  }

  if (looksLikeNetworkError(error, status)) {
    return makeClassification(
      true,
      'network',
      'The network request failed before a permanent response was received.',
      error,
      status,
    );
  }

  return makeClassification(
    false,
    'unknown',
    'The error is not known to be safely retryable.',
    error,
    status,
  );
}

export function isRetryableError(error: unknown): boolean {
  return classifyRetryableError(error).retryable;
}

export const classifyError = classifyRetryableError;
export const classifyHttpError = classifyRetryableError;
export const isRetryable = isRetryableError;

export function isRetryableStatus(status: number): boolean {
  return Number.isInteger(status) && (status === 408 || status === 425 || status === 429 || (status >= 500 && status <= 599));
}

export function toOperationError(
  error: unknown,
  classification: RetryClassification = classifyRetryableError(error),
): OperationError {
  return {
    name: errorName(error),
    message: errorMessage(error),
    ...(classification.code === undefined ? {} : { code: classification.code }),
    ...(classification.status === undefined ? {} : { status: classification.status }),
    retryable: classification.retryable,
    retryClass: classification.kind,
    ...(classification.retryAfterMs === undefined
      ? {}
      : { retryAfterMs: classification.retryAfterMs }),
  };
}

export const normalizeError = toOperationError;

function resolveMaxAttempts(options: RetryOptions): number {
  if (options.maxAttempts !== undefined && options.maxRetries !== undefined) {
    const implied = options.maxRetries + 1;
    if (options.maxAttempts !== implied) {
      throw new RetryConfigurationError(
        'maxAttempts and maxRetries must describe the same retry bound',
      );
    }
  }
  const value = options.maxAttempts ?? (options.maxRetries !== undefined ? options.maxRetries + 1 : DEFAULT_MAX_ATTEMPTS);
  if (!Number.isInteger(value) || value < 1 || value > MAX_ALLOWED_ATTEMPTS) {
    throw new RetryConfigurationError(
      `maxAttempts must be an integer between 1 and ${MAX_ALLOWED_ATTEMPTS}`,
    );
  }
  return value;
}

function resolveNumber(
  value: number | undefined,
  fallback: number,
  name: string,
): number {
  const resolved = value ?? fallback;
  if (!Number.isFinite(resolved) || resolved < 0) {
    throw new RetryConfigurationError(`${name} must be a finite non-negative number`);
  }
  return resolved;
}

function validateRetryOptions(options: RetryOptions): void {
  resolveMaxAttempts(options);
  resolveNumber(options.baseDelayMs, DEFAULT_BASE_DELAY_MS, 'baseDelayMs');
  resolveNumber(options.maxDelayMs, DEFAULT_MAX_DELAY_MS, 'maxDelayMs');
  const factor = resolveNumber(options.backoffFactor, DEFAULT_BACKOFF_FACTOR, 'backoffFactor');
  const jitter = resolveNumber(options.jitter, 0, 'jitter');
  if (factor < 1) throw new RetryConfigurationError('backoffFactor must be at least 1');
  if (jitter > 1) throw new RetryConfigurationError('jitter must be between 0 and 1');
}

function retryDelay(
  attempt: number,
  classification: RetryClassification,
  options: RetryOptions,
): number {
  const base = resolveNumber(options.baseDelayMs, DEFAULT_BASE_DELAY_MS, 'baseDelayMs');
  const maximum = resolveNumber(options.maxDelayMs, DEFAULT_MAX_DELAY_MS, 'maxDelayMs');
  const factor = resolveNumber(options.backoffFactor, DEFAULT_BACKOFF_FACTOR, 'backoffFactor');
  const jitter = resolveNumber(options.jitter, 0, 'jitter');
  if (factor < 1) {
    throw new RetryConfigurationError('backoffFactor must be at least 1');
  }
  if (jitter > 1) {
    throw new RetryConfigurationError('jitter must be between 0 and 1');
  }

  const exponential = Math.min(maximum, base * factor ** Math.max(0, attempt - 1));
  const serverHint = options.respectRetryAfter === false
    ? undefined
    : classification.retryAfterMs;
  let delay = serverHint === undefined
    ? exponential
    : Math.min(maximum, Math.max(0, serverHint));
  if (jitter > 0 && delay > 0) {
    const random = options.random ?? Math.random;
    const sample = random();
    if (!Number.isFinite(sample)) {
      throw new RetryConfigurationError('random must return a finite number');
    }
    const normalized = Math.min(1, Math.max(0, sample));
    delay = Math.min(maximum, Math.max(0, delay * (1 - jitter + 2 * jitter * normalized)));
  }
  return Math.min(maximum, Math.max(0, delay));
}

function linkExternalSignal(
  signal: AbortSignal | undefined,
  controller: AbortController,
): () => void {
  if (!signal) return () => undefined;
  const onAbort = () => controller.abort(toAbortError(signal.reason));
  if (signal.aborted) onAbort();
  else signal.addEventListener('abort', onAbort, { once: true });
  return () => signal.removeEventListener('abort', onAbort);
}

function throwForSignal(signal: AbortSignal): never {
  throw toAbortError(signal.reason);
}

async function runRetryLoop<T>(
  operation: (context: RetryAttemptContext) => T | Promise<T>,
  options: RetryOptions,
  reconciliation?: (
    context: RetryContext,
  ) => Promise<ReconciliationDecision<T>> | ReconciliationDecision<T>,
): Promise<T> {
  validateRetryOptions(options);
  const maxAttempts = resolveMaxAttempts(options);
  const controller = new AbortController();
  const unlink = linkExternalSignal(options.signal, controller);

  try {
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      if (controller.signal.aborted) throwForSignal(controller.signal);
      try {
        return await withAbort(
          () => operation({
            attempt,
            maxAttempts,
            signal: controller.signal,
          }),
          controller.signal,
        );
      } catch (error) {
        if (controller.signal.aborted) throwForSignal(controller.signal);
        const classification = classifyRetryableError(error);
        if (!classification.retryable) throw error;

        const delayMs = retryDelay(attempt, classification, options);
        const context: RetryContext = {
          error,
          classification,
          attempt,
          nextAttempt: attempt + 1,
          maxAttempts,
          signal: controller.signal,
          delayMs,
        };

        if (
          options.shouldRetry &&
          !(await withAbort(
            () => options.shouldRetry!(error, context),
            controller.signal,
          ))
        ) {
          throw error;
        }
        if (controller.signal.aborted) throwForSignal(controller.signal);

        if (reconciliation) {
          const decision = await withAbort(
            () => reconciliation(context),
            controller.signal,
          );
          if (decision.kind === 'resolved') {
            return decision.value;
          }
          if (decision.kind === 'failed') {
            throw decision.error;
          }
          if (decision.kind !== 'retry') {
            throw new RetryConfigurationError('reconcile returned an unsupported decision');
          }
        }
        if (controller.signal.aborted) throwForSignal(controller.signal);

        if (attempt >= maxAttempts) throw error;

        if (options.onRetry) {
          await withAbort(() => options.onRetry!(context), controller.signal);
        }
        if (delayMs > 0) {
          const waiter = options.wait ?? systemRetryWaiter;
          await withAbort(
            () => waiter(delayMs, controller.signal, options.scheduler),
            controller.signal,
          );
        }
      }
    }

    // The loop always returns or throws. This is only for type-checking and to
    // make an unexpected future control-flow change explicit.
    throw new RetryConfigurationError('retry loop completed without a result');
  } finally {
    unlink();
  }
}

export type IdempotentRetryOperation<T> = (
  context: RetryAttemptContext,
) => T | Promise<T>;

/**
 * Retry an idempotent operation. This function is intentionally named
 * idempotent so a create call cannot be accidentally passed to it.
 */
export function retryIdempotent<T>(
  operation: IdempotentRetryOperation<T>,
  options: RetryOptions = {},
): Promise<T> {
  return runRetryLoop(operation, options);
}

/** Explicit aliases retain the idempotent-only contract in their names/docs. */
export const withRetry = retryIdempotent;
export const executeWithRetry = retryIdempotent;

export type SignalRetryOperation<T> = (
  signal: AbortSignal,
  context: RetryAttemptContext,
) => T | Promise<T>;

/** Signal-first convenience form for adapters which pass AbortSignal to a client. */
export function retryWithSignal<T>(
  operation: SignalRetryOperation<T>,
  options: RetryOptions = {},
): Promise<T> {
  return retryIdempotent((context) => operation(context.signal, context), options);
}

export interface ReconciliationContext {
  readonly error: unknown;
  readonly classification: RetryClassification;
  readonly attempt: number;
  readonly nextAttempt: number;
  readonly maxAttempts: number;
  readonly signal: AbortSignal;
  readonly delayMs: number;
}

export type ReconciliationDecision<T> =
  | { readonly kind: 'resolved'; readonly value: T }
  | { readonly kind: 'retry' }
  | { readonly kind: 'failed'; readonly error: unknown };

export type ReconciliationCallback<T> = (
  context: ReconciliationContext,
) => ReconciliationDecision<T> | Promise<ReconciliationDecision<T>>;

export interface CreateRetryOptions<T> extends RetryOptions {
  /** Required. It must inspect the remote state before another create call. */
  readonly reconcile: ReconciliationCallback<T>;
}

function parseReconciliationDecision<T>(value: ReconciliationDecision<T>): ReconciliationDecision<T> {
  if (!value || typeof value !== 'object' || !('kind' in value)) {
    throw new RetryConfigurationError(
      'reconcile must return { kind: "resolved", value }, { kind: "retry" }, or { kind: "failed", error }',
    );
  }
  if (value.kind === 'resolved' && !('value' in value)) {
    throw new RetryConfigurationError('a resolved reconciliation decision must include value');
  }
  if (value.kind !== 'resolved' && value.kind !== 'retry' && value.kind !== 'failed') {
    throw new RetryConfigurationError('reconcile returned an unsupported decision');
  }
  if (value.kind === 'failed' && !('error' in value)) {
    throw new RetryConfigurationError('a failed reconciliation decision must include error');
  }
  return value;
}

/**
 * Retry a non-idempotent create only after a reconciliation callback has had a
 * chance to discover an already-created resource. A missing callback is a
 * configuration error and no blind create retry is performed.
 */
export function retryCreate<T>(
  operation: (context: RetryAttemptContext) => T | Promise<T>,
  options: CreateRetryOptions<T>,
): Promise<T> {
  if (!options || typeof options.reconcile !== 'function') {
    return Promise.reject(
      new RetryConfigurationError(
        'Non-idempotent create operations require a reconcile callback',
      ),
    );
  }

  return runRetryLoop(
    operation,
    options,
    async (context) => {
      const decision = parseReconciliationDecision(await options.reconcile(context));
      if (decision.kind === 'resolved') return { kind: 'resolved', value: decision.value };
      return decision;
    },
  );
}

export const retryNonIdempotent = retryCreate;
export const retryWithReconciliation = retryCreate;
export const createWithReconciliation = retryCreate;

export function retryCreateWithSignal<T>(
  operation: SignalRetryOperation<T>,
  options: CreateRetryOptions<T>,
): Promise<T> {
  return retryCreate((context) => operation(context.signal, context), options);
}

export { MAX_ALLOWED_ATTEMPTS as MAX_RETRY_ATTEMPTS };
