...
import { hashPassword, saveAdminFile } from './auth';
  ...
  return new Promise((resolve) => {
    ...
    const onData = (d: Buffer) => {
      ...
      if (s === '\n' || s === '\r' || s === '\r\n') {
        ...
        if (stdin.isTTY) (stdin as unknown as { setRawMode: (m: boolean) => void }).setRawMode(false);
    ...
    if (stdin.isTTY) (stdin as unknown as { setRawMode: (m: boolean) => void }).setRawMode(true);
    stdin.resume();
...
async function main() {
  ...
  if (save) {
    await saveAdminFile(username, hash);
